...
tls_get_peer_cert_hash(struct tls *ctx, char **hash)
{
	...
	*hash = NULL;
	...
	if (tls_hex_string((unsigned char *)d, dlen, &dhex, NULL) != 0) {
...
tls_get_peer_cert_times(struct tls *ctx, time_t *notbefore, time_t *notafter)
{
	...
	if (ctx->ssl_peer_cert != NULL) {
		...
		if ((after = X509_get_notAfter(ctx->ssl_peer_cert)) == NULL)
			goto err;
		if (asn1_time_parse((char *)before->data, before->length, &before_tm, 0)
		    == -1)
			goto err;
		if (asn1_time_parse((char *)after->data, after->length, &after_tm, 0)
		    == -1)
			goto err;
